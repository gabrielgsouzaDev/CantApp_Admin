...
export const getOrders = async (): Promise<Order[]> => {
  const response = await api.get<{ data: Order[] }>('/pedidos');
...
export const getRecentSales = async (): Promise<Order[]> => {
  ...
  const response = await api.get<{ data: Order[] }>('/pedidos?limit=5&sort=desc'); 
...
export const updateOrderStatus = async (orderId: number, newStatus: OrderStatus): Promise<Order> => {
  const response = await api.patch<any>(`/pedidos/${orderId}`, { status: newStatus });
