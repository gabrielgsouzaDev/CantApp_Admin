...
export const getOrders = async (): Promise<Order[]> => {
  const response = await api.get<Order[]>('/api/pedidos');
  // Assuming the backend returns the correct structure, if not, mapping is needed
  const sortedOrders = response.sort((a, b) => new Date(a.time).getTime() - new Date(b.time).getTime());
...
export const getRecentSales = async (): Promise<Order[]> => {
  ...
  const response = await api.get<Order[]>('/api/pedidos?limit=5&sort=desc'); 
  const sortedSales = response.sort((a, b) => new Date(b.time).getTime() - new Date(a.time).getTime());
